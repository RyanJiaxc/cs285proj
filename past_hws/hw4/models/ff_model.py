...
class FFModel(nn.Module, BaseModel):
    ...
    def get_prediction(self, obs, acs, data_statistics):
        ...
        prediction, _ = self.forward(obs, acs, self.obs_mean, self.obs_std, self.acs_mean,
    ...
    def update(self, observations, actions, next_observations, data_statistics):
        ...
        self.update_statistics(**data_statistics)
        target = (next_obs - obs - self.delta_mean) / self.delta_std
        # Hint: you should use `data_statistics['delta_mean']` and
        # `data_statistics['delta_std']`, which keep track of the mean
        # and standard deviation of the model.

        _, delta_pred_normalized = self.forward(obs, acs, self.obs_mean, self.obs_std, self.acs_mean,
                                        self.acs_std, self.delta_mean, self.delta_std)
        loss_fn = nn.MSELoss(reduction='sum')
        loss = loss_fn(delta_pred_normalized, target)
        # Hint: `self(...)` returns a tuple, but you only need to use one of the outputs.
