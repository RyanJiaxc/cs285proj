...
class MPCPolicy(BasePolicy):
    ...
    def sample_action_sequences(self, num_sequences, horizon):
        ...
        # [self.low, self.high]
        not_in_range = np.random.sample((num_sequences, horizon, self.ac_dim))
        random_action_sequences = not_in_range * (self.high - self.low) + self.low
    ...
    def calculate_sum_of_rewards(self, obs, candidate_action_sequences, model):
        ...
        """
        stacked_obs = np.stack([obs] * candidate_action_sequences.shape[0])
        predicted_obs = []  # shape N * H * D_obs
        
        for i in range(candidate_action_sequences.shape[1]):
            predicted_obs.append(stacked_obs)
            stacked_obs = model.get_prediction(stacked_obs, candidate_action_sequences[:, i, :], 
                                               self.data_statistics)
        predicted_obs = np.array(predicted_obs).transpose(1, 0, 2)  # transpose to agree with shape
        
        sum_of_rewards = [np.sum(self.env.get_reward(predicted_obs[i, :, :], 
                                                     candidate_action_sequences[i, :, :])[0]) 
                          for i in range(candidate_action_sequences.shape[0])]  # TODO (Q2)
