...
class BootstrappedContinuousCritic(nn.Module, BaseCritic):
    ...
    def update(self, ob_no, ac_na, next_ob_no, reward_n, terminal_n):
        ...
        for target_update_n in range(self.num_target_updates):
            vs_tp1 = self.forward(next_obs)
            target = reward_n + self.gamma * vs_tp1 * (1 - terminal_n)
            target = target.detach()

            for grad_step_n in range(self.num_grad_steps_per_target_update):
                loss = 1 / 2 * loss_fn(self.forward(obs), target)
